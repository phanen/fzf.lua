...
local defaults = {
  cmd = {
    _G.is_win and fn.executable('pwsh') == 1 and 'pwsh' or (fn.executable('fish') == 1 and 'fish' or vim.o.shell),
  },
  auto_close = false,
  clear_env = false,
}
...
---@return boolean
function M:is_running() return self.buf and fn.jobwait({ vim.bo[self.buf].channel }, 0)[1] == -1 and true or false end
